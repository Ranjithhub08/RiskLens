...
| 0.50 – 0.53 | Escalate | Routed to a human reviewer queue with the full explanation attached |
| > 0.53 | Flag for compliance review | Routed as high-priority, explanation + supporting SHAP chart attached |
...
The thresholds (0.50 / 0.53) are configuration, not hard-coded logic — deliberately, so you can show a judge you thought about tunability and false-positive/false-negative tradeoffs. They aren't round guesses: the model's predicted probabilities on held-out data are naturally compressed into a narrow band rather than spanning the full 0-1 range, so the thresholds were placed from that actual distribution -- Escalate starts just above the dense cluster of ordinary scores (~80th percentile), Flag starts at the beginning of the high-risk tail (~90th percentile). On the held-out test set this produces a clean risk gradient: the Clear bucket's true risky rate is 6.3%, Escalate's is 30.8%, and Flag's is 46.7%, against a 12.8% base rate -- each tier is measurably riskier than the last. An earlier version of this table used illustrative 0.40 / 0.75 values that sat almost entirely inside that compressed band, so nearly everything landed in Escalate or the manual-review fail-safe and Clear/Flag almost never fired -- a good example of why Section 12.4's threshold explorer matters: a threshold is only meaningful when checked against what the model actually outputs. FLAG_THRESHOLD itself was later moved from 0.62 to 0.53 for the same reason: `data/raw/generate_data.py`'s daily-volume generator never actually produced a genuine volume spike (`volume_change_pct` topped out at 0.50 across the whole training set even though the label rule treats anything past 0.8 as a real risk signal), so the model never learned that pattern -- fixed by adding a real spiking subset, which sharpened the model's separation enough that the old 0.62 threshold ended up above the highest score the retrained model ever produces. Nowhere in this system does a score alone freeze an account; it only ever produces a recommendation plus a reason.
...
- The gating thresholds (0.50 / 0.53) are derived from the current model's own score distribution on held-out data (Section 4.5), not from a real cost-of-false-positive vs. cost-of-false-negative analysis, which a production deployment would require -- and they would need re-checking (via the Threshold Explorer) any time the model is retrained, since they're tuned to this model's output range, not universal.
